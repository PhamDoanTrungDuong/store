	...
	return (
		<>
			<button
				...
				type="button">
				<AiTwotoneBell className="hover:fill-indigo-600 duration-200" size={25}/>
				<div className={`relative ${comments?.find((item) => item.isNoftify === true) ? "flex" : "hidden"}`}>
			...
			{/* <!-- Dropdown menu --> */}
			<div onClick={() => setIsOpen(false)} className={`${!isOpen ? "hidden" : ""} w-screen h-screen absolute top-0 left-0`}></div>
			<div
				id="dropdownNotification"
				className={`${!isOpen ? "hidden" : ""} duration-300 absolute right-14 top-14 z-20 text-black w-full max-w-sm bg-white divide-y shadow dark:bg-white divide-indigo-600 rounded-xl h-[400px] overflow-y-scroll`}
            >
						...
						return(
							...
							<div className="flex-shrink-0">
								<img
									...
									src={comment.pictureUrl === null ? "/images/empty-user.png" : comment.pictureUrl}
								...
								<div className="flex absolute justify-center items-center ml-6 -mt-5 w-5 h-5 bg-white rounded-full border border-white dark:border-gray-800">
									<AiFillHeart size={15} className="fill-red-500" />
								</div>
							</div>
							<div className="pl-3 w-full">
								<div className="text-gray-500 text-sm mb-1.5 dark:text-gray-400">
									...
									: "{comment.content}" to product {comment.productId} - {comment.productName}
								</div>
