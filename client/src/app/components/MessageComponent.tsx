	...
	return (
		<>
			<Tooltip TransitionComponent={Zoom} title="Chat With Admin">
				<div
					className="w-20 h-20 rounded-full shadow shadow-slate-400 flex justify-center items-center cursor-pointer z-50"
					onClick={handleOpen}>
					<img
			...
			{open ? (
        <>
          <div className="h-screen w-screen z-0 fixed right-0 bottom-0" onClick={handleOpen}></div>
                ...
                <div className="w-full">
                  ...
                  <div className="relative flex items-center p-3 border-b border-gray-300">
                    <img className="object-cover w-10 h-10 rounded-full"
                      src="images/admin.jpg" alt="username" />
                    <span className="block ml-2 font-bold text-gray-600">Administrator</span>
                  ...
                  <div className="relative w-full p-6 overflow-y-auto h-[465px]">
                    <ul className="space-y-2">
                      {messages && messages.map((mess: any, idx) => {
                        return (
                          <>
                            {mess.recipientUsername === user?.username ? (
                              <li className="flex justify-start">
                                <div className="relative max-w-xl px-4 py-2 text-gray-700 rounded shadow">
                                  <span className="block">{mess.content}</span>
                                </div>
                              </li>
                            ) : (
                              <></>
                            )}
                            {mess.recipientUsername === "admin" ? (
