...
const ProductDetails: React.FC = () => {
	...
	const { handleSubmit, register: register1 } = useForm<Inputs>();
	const { register: register2, watch, setValue } = useForm();
	const [valuesStar, setValuesStar] = useState<number | null>(0);
	const [colors, setColors] = useState([]);
	const [sizes, setSizes] = useState([]);
	const [orderComment, setOrderComment] = useState<any>();
	...
	// const quantity = watch("quantity", 0); // Initialize with 0
	...
	useEffect(() => {
		variantsProduct &&
			variantsProduct
				.filter(
					(item: any) =>
						item.colourValue === selectedColor &&
						item.sizeValue === selectedSize
				)
				.map((i: any) => setProductQty(i.quantity));
	}, [selectedColor, selectedSize, variantsProduct]);
	...
	var arr: number[] = [];
	orderComment &&
		orderComment.flat().map((item: any) => {
			arr.push(item.itemOrdered.productId);
		});
	...
	const hanldeUpdateCart = () => {
		if (!item || quantity > item.quantity) {
			const updatedQty = item ? quantity - item.quantity : quantity;
			if(updatedQty > productQty) {
				Swal.fire({
					icon: "warning",
					title: "Not enough stock for product variant",
					showConfirmButton: false,
					timer: 1500,
			  });
			} else {
			...
			<div className="grid md:grid-cols-2 ml-4">
				...
				<div className="max-w-[400px] mt-5 md:mt-0">
					...
					<div className="mt-8">
						<h4 className="text-3xl font-bold">
							{product.name}
							<div className="text-base text-gray-500 mr-3 pt-2">
								{product.quantityInStock} Total in
								Stock
							</div>
						</h4>
						<div className="my-4">
					...
					<div className="my-8 flex justify-between items-center">
						<div className="flex items-center gap-4 text-medium ">
							...
							{/* <button
							 ...
							 <input
								type="number"
								className="text-xl border p-2 rounded w-20"
								value={quantity}
