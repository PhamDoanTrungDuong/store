...
const ProductSearch: React.FC = () => {
  ...
  const searchProduct = t('ProSear_Search');
	return (
		<div className="relative">
			...
			<div>
				<div
					className="absolute top-1 right-5 p-2 text-zinc-500 border border-zinc-100 cursor-pointer hover:scale-125 hover:border-transparent hover:text-sky-500 hover:bg-sky-200/30 rounded-full duration-200"
					onClick={handleVoice}>
					<MdOutlineKeyboardVoice size={33} />
				</div>
				<div className="absolute top-15 right-1">
					<span className="text-sky-500">
						{isVoice === true ? "Recording..." : ""}
					</span>
					<span className="text-red-600">
						{error === true ? errorMess : ""}
					</span>
				</div>
			</div>
		</div>
